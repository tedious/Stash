...
class StashUtilitiesTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testEncode()
    {
        ...
        $this->assertEquals(Stash\Utilities::encode('String of doom!'), 'String of doom!', 'encode returned string scalar');
        $this->assertEquals(Stash\Utilities::encode(234), 234, 'encode returned integer scalar');
        $this->assertEquals(Stash\Utilities::encode(1.432), 1.432, 'encode returned float scalar');
    ...
    {
        $this->assertTrue(Stash\Utilities::decode(Stash\Utilities::encode(true), Stash\Utilities::encoding(true)), 'decode unpacked boolean \'true\'.');

        $this->assertFalse(Stash\Utilities::decode(Stash\Utilities::encode(false), Stash\Utilities::encoding(false)), 'decode unpacked boolean \'false\'');

        $string = 'String of doom!';
        $this->assertEquals(Stash\Utilities::decode(Stash\Utilities::encode($string), Stash\Utilities::encoding($string)), $string, 'decode unpacked string');

        $this->assertEquals(Stash\Utilities::decode(Stash\Utilities::encode(234), Stash\Utilities::encoding(234)), 234, 'Sdecode unpacked integer');

        $this->assertEquals(Stash\Utilities::decode(Stash\Utilities::encode(1.432), Stash\Utilities::encoding(1.432)), 1.432, 'decode unpacked float');

        $std = new stdClass();
        $this->assertEquals(Stash\Utilities::decode(Stash\Utilities::encode($std), Stash\Utilities::encoding($std)), $std, 'decode unpacked object');

        $array = array(4, 5, 7);
        $this->assertEquals(Stash\Utilities::decode(Stash\Utilities::encode($array), Stash\Utilities::encoding($array)), $array, 'decode unpacked array');
    }
    ...
    public function testDeleteRecursive()
    {
        ...
        $this->assertTrue(Stash\Utilities::deleteRecursive($dirTwo . '/test3'), 'deleteRecursive returned true when removing single file.');
        $this->assertFileNotExists($dirTwo . '/test3', 'deleteRecursive removed single file');


        $this->assertTrue(Stash\Utilities::deleteRecursive($tmp), 'deleteRecursive returned true when removing directories.');
        $this->assertFileNotExists($tmp, 'deleteRecursive cleared out the directory');

        $this->assertFalse(Stash\Utilities::deleteRecursive($tmp), 'deleteRecursive returned false when passed nonexistant directory');
    }
}
